...
\begin{center}
Table 1. Point estimates and $95\%$ confidence interval of the simulated result
\end{center}
![](./graphs in report/table1.jpg)

Table 1 demonstrates all point estimates and $95\%$ confidence intervals of three models under 1000 simulations of uncensored data with sample size of 400 and true $\beta=4$. When simulating the exponential data, the baseline model-Exponential model has the exact same point estimator of the true $\beta$, with the narrowest $95\%$ CI. Weibull and Cox share the same point estimator with slightly larger CI. In scenario 2, the Weibull model has the closest estimation of the treatment effect we expected with a narrow CI. Cox follows, with a little deviation from the true $\beta$. But the exponential model fails to fit a mean similar to the weibull. In the third scenario, both Exponential and Weibull produce large deviation from the true $\beta$ and the $95\%$ CI fails to include the ture $\beta$.  Cox model fits its own data very well.



![](./graphs in report/figure1.png)
\begin{center}
Figure 1. Association  between $\beta$  and MSE among three scenarios in uncensored data\\(A: under exponential;B:under weibull;C: undercox )
\end{center}


![](./graphs in report/figure2.png)
\begin{center}
Figure 2. Association  between $\beta$  and Bias among three scenarios in uncensored data\\(A: under exponential;B:under weibull;C: undercox )
\end{center}


From Figure 1, we observed in Scenario 1 that as the treatment effect ($\beta$) was varied the exponential model performed the best. The cox and Weibull model decreased in performance as MSE increased with increasing $\beta$. The performance of Weibull is between Exponential and Cox models. But generally, the increase of MSE under three models is still small with the greatest MSE at 0.16. Under the weibull scenario, the weibull and cox model performed the best. The MSE is nearly 0 for all treatment effects between these two models. The exponential model performed poorly with a larger treatment effect. Under the unspecified Gompertz scenario, the Cox model performs the best. Meanwhile, weibull and exponential had an increased MSE as treatment effect increased with exponential performing the worst.



In Figure 2, the bias trends among three scenarios are similar to those of MSE. Under the exponential scenario, with varied beta, the exponential performs well in keeping bias low whereas Cox and Weibull models perform worse with larger betas. In the Weibull scenario, both Cox and Weibull models perform well with keeping bias low. However, the exponential model increasingly underestimates the treatment effect as it becomes larger. With the unspecified hazard function model, the Cox model shows great performance with different treatment effects. Weibull and exponential models decrease in performance as treatment effects become larger as the bias deviates further from 0. 
...
\begin{center}
Figure 3. Association between sample size and model fitting in exponential scenario\\
(A: MSE; B: Bias)
\end{center}

The exponential model demonstrates a good fit in the exponential scenario under the uncensored data with low MSE and bias across all sample sizes. Weibull model performs better than the cox model and both MSE and bias decrease when the sample size increases. Cox model holds a similar trend in MSE and bias for censored and uncensored data. The Cox model fits the data poorly but improves as the sample size becomes larger. Overall, uncensored status performs similar to the censored.


![Figure 4](./graphs in report/figure4.png)
\begin{center}
Figure 4. Association between sample size and model fitting in weibull scenario\\
(A: MSE; B: Bias)
\end{center}

Under the weibull scenario, the weibull model fits its own data well. The exponential model performs worst with high values of MSE and bias in both censored and uncensored status. Cox model  shows a better performance than exponential when sample size is greater than 250 across the censored and uncensored status. Meanwhile, cox model fails to fit the weibull data well when sample size is less than 200, showing an exponentially decrease in the sample size ranging from 100 to 200. 


![Figure 5](./graphs in report/figure5.png)
\begin{center}
Figure 5. Association between sample size and model fitting in cox scenario\\
(A: MSE; B: Bias)
\end{center}

In the cox scenario, cox model fits the data best when the sample size is higher than 250 but fails to fit the data well with the sample size around 100. Weibull and exponential fit the data steadily without any evident bump, but the MSE and bias values are significantly large in both censored and uncensored status. Weibull model has lower MSE and bias than the cox, meaning that it holds the priority in fitting the cox data compared with exponential model. 
