...
sim_results = 
  ...
  mutate(
    ...
    output_censored = map(.x = sample_size, 
                          ~simulate(sim = 10, N = .x, beta = 4, censor = TRUE)$performance),
...
sim_results_beta = 
  ...
  mutate(
    output_uncensored = map(.x = beta, ~simulate(sim = 50, N = 100, beta = .x)$performance),
    output_censored = map(.x = beta, ~simulate(sim = 50, N = 100, beta = .x, censor = TRUE)$performance),
    output_uncensored = map(.x = output_uncensored, ~mutate(.x, censor = FALSE)),
    output_censored = map(.x = output_censored, ~mutate(.x, censor = TRUE))) 
